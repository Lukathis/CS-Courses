...
int isTmax(int x) {
  ...
  // 0x80000000 ^ 0x7FFFFFFF = 0xFFFFFFF
  // Part 2. Rule out 0xFFFFFFFF 
  return (!(~((x+1u)^x))) & (!!((x+1u)^0));
}
/* 
...
int conditional(int x, int y, int z) {
  // if x != 0 -> a1 = 0xFFFFFFFF
  // if x = 0 -> a1 = 0x00000000 
  int a1 = ~(!!x) + 1u;
  int a2 = ~a1;
  int b = y & a1;
  int c = z & a2;
...
int isLessOrEqual(int x, int y) {
  ...
  int pos_y = !(y&TMIN);
  // if x > 0 and y < 0
  int cond1 = pos_x & (!pos_y);
  // if x < 0 and y > 0
  int cond2 = (!pos_x) & pos_y;

  int nx = ~x+1;
  int delta = y + nx;
  int cond_less = !(delta&TMIN);
  int cond_eq = !(x^y);
  return (!cond1)&(cond2|cond_less|cond_eq);
}
//4
/* 
...
int logicalNeg(int x) {
  unsigned endWith1 = (1 & x);
  unsigned notZero = ((((unsigned)x >> 1) + (~1u>>1)) >> 31);
  return !(endWith1 | notZero);
}
...
int howManyBits(int x) {
  ...
  // improvement
  int positive = x >> 31; // 0x0 is positive
  unsigned norm = (~positive & x) + (positive & (~x));
  int firstOne = 33 - (!norm + !(norm>>1) + !(norm>>2) + !(norm>>3) + !(norm>>4) + !(norm>>5) + !(norm>>6) + !(norm>>7) + !(norm>>8) + !(norm>>9) + !(norm>>10)
                             + !(norm>>11) + !(norm>>12) + !(norm>>13) + !(norm>>14) + !(norm>>15) + !(norm>>16) + !(norm>>17) + !(norm>>18) + !(norm>>19) + !(norm>>20)
                             + !(norm>>21) + !(norm>>22) + !(norm>>23) + !(norm>>24) + !(norm>>25) + !(norm>>26) + !(norm>>27) + !(norm>>28) + !(norm>>29) + !(norm>>30) + !(norm>>31));
  return firstOne;
